...
test("uses white light paper while preserving the dark palette", async ({ page }) => {
  ...
  const helperText = page.getByText("Busca palabras o frases en todo el manual.")
  ...
  await expect(helperText).toHaveCSS("color", "rgb(92, 95, 90)")
  ...
  await expect(helperText).toHaveCSS("color", "rgb(163, 164, 157)")
