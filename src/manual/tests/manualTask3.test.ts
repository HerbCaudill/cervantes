...
describe("verified Task 3 manual content", () => {
  it("keeps all maps, figures, callouts, and the table with their source topics", () => {
    const contentByTopic = new Map(
      topics.map(topic => [
        ...
        topic.blocks.map(block => {
          ...
          if (block.type === "callout") return getText(block.blocks, "paragraph").join(" ")
          return block.type
        }),
      ]),
    )
    ...
    expect(
      contentByTopic.get("task-3-accidentes-geograficos-mas-importantes-de-espana")?.join(" "),
    ).toContain("La meseta es una extensa llanura")
    expect(
  ...
  it("preserves source typography and apparent source errors without extraction artifacts", () => {
    ...
    const callouts = blocks.flatMap(block =>
      block.type === "callout" ? getText(block.blocks, "paragraph") : [],
    )
    ...
    expect(callouts).toContain(
      "A diferencia de los climas que podemos encontrar en la Península, en las islas Canarias podemos encontrar temperaturas suaves todo el año, con una media de 22ºC.",
    )
